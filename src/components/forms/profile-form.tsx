...
import { BandwidthProfile } from "@/lib/types";
...
import { Loader2, ArrowLeft, Zap, ArrowDown, ArrowUp } from "lucide-react";
...
export function ProfileForm({ initialData, isEditing = false }: ProfileFormProps) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Menentukan batasan kecepatan rate-limit yang akan dikirim via RADIUS attribute (<code>Mikrotik-Rate-Limit</code>).
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="rateLimitDown" className="flex items-center gap-1.5 text-blue-600 dark:text-blue-400">
                <ArrowDown className="h-4 w-4" />
                Batas Kecepatan Download (Mbps) <span className="text-rose-500">*</span>
              </Label>
              <Input
                id="rateLimitDown"
                type="number"
              ...
              {errors.rateLimitDown && (
                <p className="text-xs text-rose-500">{errors.rateLimitDown.message}</p>
              )}
            </div>

            <div className="space-y-2">
              <Label htmlFor="rateLimitUp" className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400">
                <ArrowUp className="h-4 w-4" />
                Batas Kecepatan Upload (Mbps) <span className="text-rose-500">*</span>
              </Label>
              <Input
                id="rateLimitUp"
                type="number"
              ...
              />
              {errors.rateLimitUp && (
                <p className="text-xs text-rose-500">{errors.rateLimitUp.message}</p>
              )}
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="description">Deskripsi & Catatan Paket (Opsional)</Label>
            <Input
              id="description"

...
export default function ProfilesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(`Profile '${deleteTarget.name}' berhasil dihapus`);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 sm:text-3xl">
            Profil Layanan (PPP & Hotspot)
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Konfigurasi gateway PPP/Hotspot, IP Pool, DNS, dan queue pada Router
            MikroTik.
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Konfigurasi interface PPP & Hotspot yang diterapkan pada router
            target.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <EmptyState
              icon={Radio}
              title="Belum ada Profil Layanan"
      ...
      <ConfirmDialog
        ...
        title="Hapus Profile?"
        description={`Apakah Anda yakin ingin menghapus Profile '${deleteTarget?.name}'? Tindakan ini tidak dapat dibatalkan.`}
